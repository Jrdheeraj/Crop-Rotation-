...
function createEnhancedFloatingElements() {
    ...
    const elementTypes = [
        { geometry: new THREE.SphereGeometry(0.08, 8, 6), material: 'seeds', count: 30 },
        { geometry: new THREE.ConeGeometry(0.06, 0.2, 6), material: 'leaves', count: 20 },
        { geometry: new THREE.CylinderGeometry(0.03, 0.03, 0.3, 8), material: 'stems', count: 15 },
        { geometry: new THREE.TetrahedronGeometry(0.05), material: 'flowers', count: 10 }
    ];

    const materials = {
        seeds: new THREE.MeshLambertMaterial({ color: 0x8B4513 }),
        leaves: new THREE.MeshLambertMaterial({ color: 0x228B22 }),
        stems: new THREE.MeshLambertMaterial({ color: 0x6B8E23 }),
        ...
        if (child.userData && child.userData.type) {
            ...
            switch (data.type) {
                case 'seeds':
                    ...
                    child.position.y = data.originalY + Math.sin(time * data.speed + data.phase) * data.amplitude;
                ...
                case 'stems':
                    // Vertical growth-like motion
                    child.position.y = data.originalY + Math.sin(time * data.speed + data.phase) * data.amplitude * 0.5;
                    child.rotation.y += data.rotationSpeed.y;
                    break;
